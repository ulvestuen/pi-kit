...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from tldraw's `server.json` in the platform app-data directory. The bundled helper locates it automatically; set `TLDRAW_SERVER_JSON` to override the location.
...
```bash
# macOS default; set TLDRAW_SERVER_JSON explicitly on other platforms.
...
A ready-made helper ships with this skill at `{baseDir}/tq.mjs`. Invoke it as `node "{baseDir}/tq.mjs" <METHOD> <path> [body]` — it re-reads the port and token from `server.json` itself on every call, so you never handle the token or the fresh-shell env problem. A body starting with `{` is sent as JSON; anything else as raw `text/plain`:

```bash
node "{baseDir}/tq.mjs" POST /api/search '{"code":"return await api.getDocs()"}'
node "{baseDir}/tq.mjs" POST /api/doc/DOC_ID/exec 'return editor.getCurrentPageShapes().length'
node "{baseDir}/tq.mjs" GET  /api/doc/DOC_ID/script-status
```
...
If the helper cannot be used, fall back to raw `curl` with the `PORT`/`TOKEN` reads shown above. The raw-`curl` examples below stay in explicit form so each request is visible; translate any to `node "{baseDir}/tq.mjs" <METHOD> <path> [body]`.
...
Shown as raw `curl`; `node "{baseDir}/tq.mjs" <METHOD> <path> [body]` is the shorter equivalent that handles the port and token for you.
