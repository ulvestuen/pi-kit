...
3. Follow the AUTOMATED prompt returned by `lykkja_start` and every later
   `lykkja_checkpoint` result. Do not wait for another user message between
   PLAN, DO, CHECK, and ACT.
4. Loop. Each pass: PLAN the single next step, DO it, CHECK every criterion with
...
5. When `lykkja_checkpoint` returns ITERATING, immediately fix the weakest
   criterion it names first, then loop again. Only stop when it returns FINAL.
6. On FINAL, print `FINAL`, summarize the result, and stop. If the loop is
